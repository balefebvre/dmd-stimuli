...
%   Detailed explanation goes here
    ...
    % ...
    n_ante = ceil(args.d_ante * args.dmd_frame_rate);
    lum_ante = zeros(n_ante, 1);
    ...
    n_post = ceil(args.d_post * args.dmd_frame_rate);
    lum_post = zeros(n_post, 1);
    lum_stim = [
        lum_ante;
        lum_step;
        lum_freq_chirp;
    ...
    % Plot control figure.
    figure('visible', 'off');
    x = (1:length(lum_stim)) / args.dmd_frame_rate;
    y = lum_stim;
    plot(x, y, 'k-');
    xlim([min(x), max(x)]);
    ylim([min(y), max(y)]);
    ...
    % Compute adaptation intensity.
    if args.adaptation_intensity < 0.0
        args.adaptation_intensity = median(lum_stim);
    elseif args.adaptation_intensity < 1.0
        args.adaptation_intensity = 256.0 * args.adaptation_intensity;
    else
        args.adaptation_intensity = 1.0 * args.adaptation_intensity;
    end
    disp(['Adaptation intensity: ', num2str(args.adaptation_intensity)]);
    
    % Generate full-field profile used at the beginning.
    n_init = ceil(args.initial_adaptation_duration * args.dmd_frame_rate);
    lum_init = args.adaptation_intensity * ones(n_init, 1);
    ...
    lum_trial = args.adaptation_intensity * ones(n_trial, 1);
    lum_trial = uint8(lum_trial);
